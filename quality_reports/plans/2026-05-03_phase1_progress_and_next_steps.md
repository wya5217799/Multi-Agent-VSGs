...
| **P0-1 L1+L2+H1 (post-fix serial)** | `python -m probes.kundur.probe_state --phase 1,2,3,4 --workers 1 --t-warmup-s 5.0 --sim-duration 3.0 --output-dir results/harness/kundur/probe_state/p2_post_l2h1_serial` (under v3 Discrete profile) | 🟡 PARTIAL — parallelization OK, LoadStep physics deferred | Wall 2420.6s (40.3 min). 15/15 dispatches OK. Hybrid pinned to G2 → 0.211105 (vs pre-fix 0.173). LoadStep dispatches all 0.108096 (silent no-op exposed; sim log warning: `LoadStep_amp_bus15 ... nontunable parameter ... initialized with Fast Restart`). G1/G2/G3/G5 PASS, G4 REJECT, G6 PENDING. Snapshot `state_snapshot_20260504T000732.json`. |
| **P0-1 E2E parallel (4 worker)** | same as above with `--workers 4 --output-dir p2_post_l2h1_parallel` | ✅ ALL_GATES_PASS for parallelization | Wall 761.0s = 12.7 min, **3.18× speedup**. **GATE-PHYS 15/15 bit-exact** (max_delta 0.00e+00 abs at TOL 1e-9), GATE-WALL PASS (vs 1192s threshold), GATE-G15 PASS (verdicts identical), GATE-LIC PASS (4 workers exit 0). LoadStep silent no-op deterministic across modes. Snapshot `state_snapshot_20260504T002521.json`. P2 ADR PARTIAL → FULL_PASS. |
