...
# regressions only — set KUNDUR_MODEL_PROFILE env-var to override.
DEFAULT_KUNDUR_MODEL_PROFILE = (
    Path(__file__).resolve().parent / "model_profiles" / "kundur_cvs_v3.json"
)
...
# ========== Kundur-specific warmup override (production locked) ==========
# T_WARMUP = 10.0 s, locked post-credibility-close 2026-04-28.
# Paper does not specify warmup duration. v3 Phasor solver inductor IC
# requires ~10 s for residual |omega - 1| to settle below 0.5 mHz across
# all 7 sources (P3.3b verdict 2026-04-26 + post_task_mini 2026-04-28
# both confirm). Below 5 s, residual contaminates r_f reward shaping.
# v2 kundur_cvs path historically ran on T_WARMUP=3.0; promoting to 10.0
# is acceptable for both (v2 benefits from extra settle margin; per-episode
# wall-clock cost uniform).
T_WARMUP = 10.0
...
# ========== Reward (Kundur-specific, locked post-credibility-close) ==========
...
# Disturbance magnitude range (locked post-credibility-close 2026-04-28).
# Raised DIST_MAX from 0.5 → 1.0 sys-pu (≈100 MW) after 2026-04-28 paper_eval
# no-control run produced cum_unnorm = -6.11 vs paper -15.20. The gap is highly
# correlated with disturbance magnitude — DIST_MAX=0.5 likely under-excites the
# system (Phase B verified 248 MW LS1 trip stable on M=24/D=4.5 baseline; Phase D
# verified D-axis 18× tau leverage under paper-faithful LS1 at 248 MW). The
# present change lifts the training cap to 1.0 sys-pu and is followed by a
# no-control paper_eval re-run to calibrate. We do NOT claim DIST_MAX alone
# accounts for the full -6.11 → -15.20 deficit; remaining mismatch may also
# stem from disturbance type (LoadStep paper-faithful vs Pm-step proxy) and
# scenario distribution.
# Earlier history: DIST_MAX=1.5 once gave peak≈14.4 Hz, saturating IntW on
