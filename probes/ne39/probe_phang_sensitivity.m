...
% 模型: NE39bus_v2
% 检查: phAng 参数注入与 Pe/omega 反馈通路健康检查（闭环/开环对比）
% 关联问题: NE39 相角控制是否影响有功，warmup 初始化是否保留 phAng 设定
...
function probe = local_reset_and_step(model_name, agent_ids, sbase_va, cfg, ...
    ...
    [state, status] = slx_step_and_read(model_name, agent_ids, M_values, ...
...
function probe = local_reset_and_two_steps(model_name, agent_ids, sbase_va, cfg, ...
    ...
    [first, status1] = slx_step_and_read(model_name, agent_ids, M_values, ...
        D_values, t_warmup + dt_probe, sbase_va, cfg, pe_prev, delta_prev);
    if ~status1.success
    ...
    [second, status2] = slx_step_and_read(model_name, agent_ids, M_values, ...
