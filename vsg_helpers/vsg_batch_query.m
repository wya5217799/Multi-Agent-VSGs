function result = vsg_batch_query(model, block_paths)
%VSG_BATCH_QUERY  Query DialogParameters for multiple blocks in one IPC call.
%
...
%     After:  1 round-trip, structured output only
    ...
    if ~bdIsLoaded(model)
        error('vsg_batch_query: model ''%s'' is not loaded. Call load_system first.', model);
    end
