function build_kundur_cvs_v3_discrete_test()
%BUILD_KUNDUR_CVS_V3_DISCRETE_TEST  Day 0 dry-run copy (Option G plan, 2026-04-30).
% Discrete-solver swap dry-run only — predicts A→B upgrade cost before
% committing to Phase 3 B fallback. Builds `kundur_cvs_v3_disc_test.slx`
% with powergui SimulationMode='Discrete' + SampleTime=50e-6.
% NOT for training. Delete after Day 0 GO/NO-GO.
%
% Original docstring follows (still applies):
...
mdl       = 'kundur_cvs_v3_disc_test';   % Day 0 dry-run only (Option G plan §0.1)
out_dir   = fileparts(mfilename('fullpath'));
out_slx   = fullfile(out_dir, [mdl '.slx']);
ic_path   = fullfile(fileparts(out_dir), 'kundur_ic_cvs_v3.json');  % shared IC OK
...
% Discrete solver dry-run (Option G plan §0.1) — was: Phasor 50 Hz
% Goal: discover whether existing block topology compiles + sims under
% Discrete + SampleTime=50e-6, OR which blocks need Discrete-Time variants.
add_block('powerlib/powergui', [mdl '/powergui'], 'Position', [20 20 100 60]);
set_param([mdl '/powergui'], 'SimulationMode', 'Discrete', ...
    'SampleTime', '50e-6', 'frequency', '50');
% Solver: keep variable-step initially; if Discrete blocks demand fixed-step,
% the compile error from Day 0 §0.3 will tell us to switch (Day 0 is
% precisely about discovering this kind of friction).
