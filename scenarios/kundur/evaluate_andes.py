...
MODEL_DIR = "results/andes_models_v2"  # v2: 标准SAC, batch=256, 校准参数
...
def plot_training_curves():
    ...
    for rdir in ["results/andes_models_v2"]:
...
def plot_comm_failure_test(agents, n_test=20):
    ...
    env_fail.reset(delta_u={"PQ_Bus14": -2.48})
...
def plot_comm_delay_test(agents, n_test=20):
    ...
    env_delay = AndesMultiVSGEnv(random_disturbance=False, comm_fail_prob=0.0,
                                 comm_delay_steps=1)
    env_delay.reset(delta_u={"PQ_Bus14": -2.48})
...
if __name__ == "__main__":
    ...
    plot_load_step(agents, "Load Step 1",
                   delta_u={"PQ_Bus14": -2.48},  # Bus 14 减载 248 MW
    ...
    plot_load_step(agents, "Load Step 2",
                   delta_u={"PQ_Bus15": 1.88},  # Bus 15 增载 188 MW
