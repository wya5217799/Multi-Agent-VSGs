...
The recompile is triggered by FastRestart off→on inside vsg_warmup.m.
...
TDD RED → GREEN:
  RED : _fr_compiled attribute missing, vsg_warmup called without do_recompile arg
  GREEN: all pass after changes to SimulinkBridge + vsg_warmup.m
"""
import pytest
...
def _make_bridge_with_mock_session():
    ...
    mock_eng = MagicMock()
    # vsg_warmup returns nothing (nargout=0)
    mock_eng.vsg_warmup = MagicMock(return_value=None)
...
class TestWarmupPassesRecompileFlag:
    ...
    def test_first_warmup_passes_do_recompile_true(self, mock_me):
        ...
        # vsg_warmup should have been called with exactly 3 positional args:
        #   (model_name, duration, do_recompile)
        # The 3rd arg must be True on the first call.
        calls = mock_eng.vsg_warmup.call_args_list
        assert len(calls) >= 1, "vsg_warmup must be called at least once"
        positional_args = calls[0][0]  # first call, positional args tuple
        assert len(positional_args) == 3, (
            f"vsg_warmup must receive 3 positional args (model, duration, do_recompile), "
    ...
    def test_second_warmup_passes_do_recompile_false(self, mock_me):
        ...
        calls = mock_eng.vsg_warmup.call_args_list
        assert len(calls) >= 2, "vsg_warmup must be called at least twice"
        second_call_args = calls[1][0]
        assert len(second_call_args) == 3, (
            f"Second vsg_warmup call must still pass 3 args, got {len(second_call_args)}"
        )
