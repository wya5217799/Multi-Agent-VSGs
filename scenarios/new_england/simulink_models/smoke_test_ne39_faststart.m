...
%   2. vsg_warmup (NE39 5-arg mode) completes without error
%   3. vsg_step_and_read advances 1 step (t=0.5 → 0.7 s) without error
...
%   addpath(genpath('C:\Users\27443\Desktop\Multi-Agent  VSGs\vsg_helpers'))
...
% ── Step 3: vsg_warmup (NE39 5-arg mode) ─────────────────────────────────
fprintf('[3] Running vsg_warmup (t_warmup=0.5s)...\n');
...
[w_state, w_status] = vsg_warmup(mdl, agent_ids, sbase_va, cfg, ip);

if ~w_status.success
    fprintf('  FAIL: vsg_warmup error: %s\n', w_status.error);
    return;
end
