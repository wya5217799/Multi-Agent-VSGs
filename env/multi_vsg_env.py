...
class MultiVSGEnv:
    ...
    def step(self, actions):
        ...
        # 4. 计算奖励
        rewards = self._compute_rewards(result, delta_H, delta_D)
    ...
    def _compute_rewards(self, state, delta_H, delta_D):
        ...
        训练期间: ΔH_avg = ΔD_avg = 0 → r^h = r^d = 0
        所以 reward = φ_f * r^f
        """
        rewards = {}
        for i in range(self.N):
            ...
            for j in neighbors:
                eta_j = 1 if self.comm.is_link_active(i, j) else 0
                if eta_j:
                    omega_c_j = state['omega'][j]
                    r_f -= (omega_c_j - omega_bar) ** 2

            # === Eq. (17-18): 训练期间 ΔH_avg = ΔD_avg = 0 ===
            r_h = 0.0
            r_d = 0.0
