...
\begin{figure}[!t]
  ...
  \caption{Checkpoint-trajectory of agent~a1 ablation share (seed~42,
  ...
  probe conducted under legacy $p_{\text{cf}} = 0.0$; five-seed mean
  a1 share at corrected $p_{\text{cf}} = 0.1$ is $64\,\%$ (range
  $54.6$--$74.7\,\%$, see \cref{fig:agent-share}). Qualitative pattern
  (early lock by episode~100) is unchanged.}
  \label{fig:ckpt-trajectory}
\end{figure}

\paragraph*{Figure provenance note.}
\emph{Figures~\ref{fig:agent-share} and~\ref{fig:cumrf} were
regenerated on 2026-05-04 with paper-faithful $p_{\text{cf}} = 0.1$
data sources
...
and~\ref{fig:worst} use legacy $p_{\text{cf}} = 0.0$ data generated
prior to the comm-fail bug fix (2026-05-04); their qualitative patterns
are explicitly verified against post-fix data in the cited audit
reports
(\texttt{quality\_reports/audits/2026-05-04\_*.md}).
All numerical claims in tables and text use post-fix,
$p_{\text{cf}} = 0.1$ values throughout.}
...
We have ruled out hyperparameter under-tuning as an explanation for
the observed null result: a single-seed $\pm 3\times$ neighborhood
...
multi-agent decoration finding is therefore robust to hyperparameter
choice within the tested neighborhood; the burden of explanation
shifts to backend characteristics or architectural choices not
addressed here.
