...
trained with one quarter of the network parameters matches DDIC
within 7.5\,\% at $n = 3$ seeds (DECORATIVE\_CONFIRMED, with the
caveat that the shared-parameter baseline is at $n = 3$ vs.\ DDIC at
$n = 5$); (iii) actor-warmstart from the shared policy reduces
seed-to-seed variance by 14.8\,\% but degrades the mean by 7.9\,\%
(WARMSTART\_WORSE), confirming the imbalance is structural rather
than initialization-driven. We additionally identify and fix
...
\begin{enumerate}
  ...
  \item Three converging lines of evidence
        ...
        parameters matches the four-agent DDIC at $n = 3$ seeds.
        We label this finding \emph{architecturally redundant on
        the ANDES phasor-equilibrium backend} and discuss its scope
        in \cref{sec:rootcause,sec:limits}.
...
\textbf{DECORATIVE\_CONFIRMED} with two explicit caveats: (a) the
shared-parameter comparison is at $n = 3$ vs.\ DDIC at $n = 5$, an
acknowledged sample-size asymmetry that is the highest-priority
follow-up (see \cref{sec:limits}); (b) the shared-parameter actor
processes four agents' transitions per environment step, so the
\emph{network-parameter} count is $1/4$ that of DDIC but the
\emph{gradient-update} count per episode is comparable. Within
these caveats, the multi-agent architecture provides no measurable
advantage over a single-actor policy at matched training budget on
the ANDES Kundur 4-VSG system.
...
DDIC (\cref{tab:phase9}, $n = 3$); (iii)~shared-policy warmstart
preserves the mean within $\pm 8\,\%$ but does not improve it
(\cref{tab:phase10}, $n = 3$). On the ANDES Kundur four-VSG system at
the operating point we tune, the four-agent architecture contributes
no statistically measurable advantage over a one-agent shared policy.
The scope of this triangulation is bounded: the shared-parameter and
warmstart comparisons are at $n = 3$ (acknowledged in \cref{sec:limits});
extending Phase~9 to $n = 5$ is the highest-priority follow-up and
would close the sample-size asymmetry with the DDIC--adaptive
comparison.
...
\begin{enumerate}
  ...
  \item The multi-agent architecture provides no measurable
        ...
        parameters achieves equivalent performance at $n = 3$. We
        scope this finding to the ANDES Kundur 4-VSG configuration
        with paper-deviating reward weights and action range; it does
        not generalize to multi-agent VSG control on EMT backends or
        at paper-faithful hyperparameters (see \cref{sec:limits}).
...
\begin{itemize}
  ...
  \item \emph{Sample-size asymmetry on the architectural-redundancy
        comparison}: Phase~9 shared-parameter SAC was trained at
        $n = 3$ seeds while Phase~4 DDIC was trained at $n = 5$.
        The DECORATIVE\_CONFIRMED finding (\cref{sec:rootcause})
        therefore rests on a smaller sample than the DDIC--adaptive
        tie claim. Three converging lines of evidence (per-agent
        ablation, shared-parameter, warmstart) collectively support
        the conclusion, but extending Phase~9 to matched $n = 5$ is
        the highest-priority follow-up and the only revision that
        would convert "decorative consistent with" into "decorative
...
mean dominance, a shared-parameter SAC matching DDIC at $n = 3$ with
$1/4$ the network parameters, and a warmstart pilot that redistributes
dominance without improving the mean -- support the conclusion that
the multi-agent architecture provides no measurable advantage
\emph{on this system at the operating point we tune}, scoped to
the ANDES phasor-equilibrium backend with paper-deviating reward
weights and action range.
...
and should extend the architectural-redundancy comparison
(Phase~9 shared-parameter) to $n = 5$ to match the DDIC--adaptive
sample size.
