...
%SLX_WARMUP  Episode reset helper -- shared by Kundur and NE39 scenarios.
%
% Kundur (2-arg) mode:
...
%   status fields: success, error, elapsed_ms
    ...
    %   It must be computed the same way slx_step_and_read.m does:
    %       phAngCmd = wrap(init_phang(k) + gain * wrap(delta_deg(k)))
    %   cfg.phase_command_mode / init_phang / phase_feedback_gain carry
    %   the model-specific values from BridgeConfig — no model-name strcmp needed.
    for i = 1:N
        idx = agent_ids(i);
    ...
    status.elapsed_ms = toc * 1000;
    fprintf('RESULT: slx_warmup (NE39) done in %.0f ms, omega=[%s]\n', ...
...
%
    ...
    if do_recompile
        ...
        try
            ...
            error('slx_warmup: FastRestart enable failed: %s', ME.message);
        end
    else
