...
修订说明:
  - H_ES0: 5→3, D_ES0: 10→2 使系统欠阻尼, 产生明显频率振荡
  - 扰动范围: 5~20 p.u. 使频率偏差达到 0.3~0.6 Hz (匹配论文)
  - ΔH/ΔD 范围按比例缩小, 适配新的 H/D 基值
...
# ═══════════════════════════════════════════════════════
#  VSG 基础参数
#  H=3, D=2 → 欠阻尼 (ζ<1), 产生明显频率振荡
#  (原 H=5,D=10 导致过阻尼, 无振荡)
# ═══════════════════════════════════════════════════════
H_ES0 = np.array([3.0, 3.0, 3.0, 3.0])    # 基础虚拟惯量 (s)
D_ES0 = np.array([2.0, 2.0, 2.0, 2.0])    # 基础虚拟阻尼 (p.u.)

# ═══════════════════════════════════════════════════════
#  动作空间 — 惯量/阻尼修正量范围 (Section IV-B)
#  按 H_ES0/D_ES0 基值等比缩放
#  确保 H_es = H_ES0 + ΔH > 0 (即 ΔH > -3)
# ═══════════════════════════════════════════════════════
DH_MIN, DH_MAX = -2.5, 15.0      # ΔH 范围 → H ∈ [0.5, 18]
DD_MIN, DD_MAX = -1.5, 20.0      # ΔD 范围 → D ∈ [0.5, 22]
...
# ═══════════════════════════════════════════════════════
DISTURBANCE_MIN = 5.0    # 最小扰动功率 (p.u.)
DISTURBANCE_MAX = 20.0   # 最大扰动功率 (p.u.)
